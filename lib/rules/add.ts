...
/**
 * A rule array that also carries a compiled RegExp of all its entries.
 * The `re` property is attached dynamically and used for directory matching.
 */
interface RuleList extends Array<string> {
  re?: RegExp;
}

/**
 * The container object holding ignore and watch rule lists.
 * Both lists are `RuleList` arrays that may carry a `.re` RegExp property.
 */
...
/** The two valid rule categories that can be passed to `add`. */
type RuleCategory = 'ignore' | 'watch';
 ...
 * ### Example:
 *
 *     var rules: Rules = { watch: [], ignore: [] };
 *     add(rules, 'watch', '*.js');
 *     add(rules, 'ignore', '/public/');
 *     add(rules, 'watch', ':(\d)*\.js'); // note: string based regexp
 *     add(rules, 'watch', /\d*\.js/);
 *
 * @param rules    containing `watch` and `ignore`. Also updated during execution
 * @param which    must be either "watch" or "ignore"
 * @param rule     the actual rule.
 */
function add(rules: Rules, which: RuleCategory, rule: string | RegExp | string[]): void {
  if (!({ ignore: 1, watch: 1 } as Record<string, number>)[which]) {
    throw new Error('rules/index.js#add requires "ignore" or "watch" as the ' +
      'first argument');
  }

  if (Array.isArray(rule)) {
    rule.forEach(function (r: string) {
      add(rules, which, r);
    });
    return;
  }
  ...
  if (regexp) {
    ...
    var re = rules[which].map(function (rule: string) {
...
module.exports = add;
export type { RuleList, RuleCategory };
export default add;
