...
function load(settings, options, config, callback) {
  ...
  loadFile(options, config, utils.home, function (options) {
    ...
    loadFile(options, config, process.cwd(), function (options) {
      ...
      // Before merging, save the config file's ignore so we can combine
      // it with CLI ignore. The merge function treats arrays atomically
      // (source replaces target), but for ignore we want CLI patterns
      // to be added on top of config file patterns, not replace them.
      var configFileIgnore = Array.isArray(options.ignore)
        ? options.ignore.slice(0)
      ...
      // If CLI specified ignore patterns AND a config file supplied ignore
      // patterns, combine them so both sets are honoured. Without this,
      // the merge above replaces the config file ignore entirely, which
      // means the config-file patterns silently disappear when the user
      // passes --ignore on the command line (see issue #2).
      // The flag _combineIgnore is set by config.load() to distinguish the
      // real CLI flow from direct callers of load() in tests.
      if (config._combineIgnore &&
          settings.ignore && settings.ignore.length > 0 &&
          configFileIgnore.length > 0) {
        configFileIgnore.forEach(function (pattern) {
          if (options.ignore.indexOf(pattern) === -1) {
