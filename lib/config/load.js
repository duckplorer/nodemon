...
// Known valid keys that can appear in nodemon.json or nodemonConfig.
// Derived from defaults, CLI parse options, exec options, and event hooks.
var knownKeys = [
  // from defaults.js
  'restartable', 'colours', 'execMap', 'ignoreRoot', 'watch', 'stdin',
  'runOnChangeOnly', 'verbose', 'signal', 'stdout', 'watchOptions',
  // from CLI parse / config
  'ignore', 'delay', 'exec', 'script', 'ext', 'env', 'cwd', 'legacyWatch',
  'spawn', 'quiet', 'nodeArgs', 'pollingInterval', 'exitCrash', 'args',
  'execArgs', 'configFile', 'noUpdateNotifier', 'dump',
  // alternative spellings
  'watch_interval', 'watchInterval', 'colors', 'exitcrash',
  // event hooks
  'events',
  // package.json wrapper key (appears after unwrapping, harmless to allow)
  'nodemonConfig',
];

// Expected types for known keys (used for value-type validation).
// Only keys with a clear expected type are listed; others are skipped.
var expectedTypes = {
  restartable: 'string|boolean',
  colours: 'boolean',
  colors: 'boolean',
  execMap: 'object',
  ignoreRoot: 'array|string',
  watch: 'array|boolean',
  stdin: 'boolean',
...
/**
 * Validates the parsed settings from a config file and emits warnings for
 * unknown keys or values whose type does not match the expected schema.
 *
 * @param {Object} settings  parsed JSON from nodemon.json / nodemonConfig
 * @param {String} filename  path shown in warning messages
 */
function validateConfig(settings, filename) {
  ...
  Object.keys(settings).forEach(function (key) {
    ...
    var allowed = expectedTypes[key];
    if (!allowed) {
      return; // no type constraint recorded – skip
    }

    var got = actualType(settings[key]);
    if (allowed.split('|').indexOf(got) === -1) {
      utils.log.fail(
        'invalid value type for "' + key + '" in ' + filename +
        ', expected ' + allowed + ' but got ' + got
      );
    }
  });
}
...
function loadFile(options, config, dir, ready) {
  ...
  fs.readFile(filename, 'utf8', function (err, data) {
    ...
    // validate the settings against the known schema and warn about problems
    var settingsToValidate = settings;
    if (filename.endsWith('package.json') && settings.nodemonConfig) {
      settingsToValidate = settings.nodemonConfig;
    }
    if (!filename.endsWith('package.json') || settings.nodemonConfig) {
      validateConfig(settingsToValidate, filename);
    }
