...
function loadFile(options, config, dir, ready) {
  ...
  fs.readFile(filename, 'utf8', function (err, data) {
    ...
    // validate settings from config files (not package.json)
    if (!filename.endsWith('package.json')) {
      validate(settings, filename);
    }
...
function loadPackageJSON(config, ready) {
  ...
  return loadFile(packageLoadOptions, config, dir, (settings) => {
    var nodemonConfig = settings.nodemonConfig || {};
    if (settings.nodemonConfig) {
      validate(nodemonConfig, filename + ' > nodemonConfig');
    }
    ready(nodemonConfig);
  });
}
