...
function load(settings, options, config, callback) {
  ...
  loadFile(options, config, utils.home, function (options) {
    ...
    loadFile(options, config, process.cwd(), function (options) {
      // Preserve config file ignore values before CLI merge, since merge
      // replaces arrays entirely and we need CLI ignore to be additive
      var configIgnore = (config._combineIgnore &&
        Array.isArray(options.ignore)) ? options.ignore.slice(0) : [];
      ...
      // When called via config.load (real CLI usage), CLI --ignore patterns
      // should be combined with config file ignores, not replace them.
      // Without this, passing --ignore on the CLI when a nodemon.json
      // exists causes the config file's ignore entries to be silently lost.
      if (configIgnore.length > 0 && settings.ignore &&
          settings.ignore.length > 0) {
        var cliIgnore = Array.isArray(settings.ignore) ?
          settings.ignore : [settings.ignore];
        options.ignore = cliIgnore.concat(
          configIgnore.filter(function (item) {
            return cliIgnore.indexOf(item) === -1;
          })
        );
      }
