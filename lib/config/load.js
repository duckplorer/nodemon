...
var validateConfig = require('./validate');
...
function loadFile(options, config, dir, ready) {
  ...
  fs.readFile(filename, 'utf8', function (err, data) {
    ...
    // validate settings from config files for unknown keys or invalid types
    if (filename.endsWith('package.json')) {
      if (settings.nodemonConfig) {
        validateConfig(settings.nodemonConfig, filename);
      }
    } else {
      validateConfig(settings, filename);
    }
