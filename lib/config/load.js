...
function load(settings, options, config, callback) {
  ...
  loadFile(options, config, utils.home, function (options) {
    ...
    loadFile(options, config, process.cwd(), function (options) {
      // Capture the config file's ignore list before merging CLI settings,
      // because merge replaces arrays rather than concatenating them.
      // CLI --ignore values should be additive to config file ignore values.
      var configIgnore = options.ignore || [];
      ...
      // If CLI provided ignore values, combine them with config file ignore
      // values so that --ignore is additive rather than replacing the config
      if (settings.ignore && settings.ignore.length > 0 &&
          configIgnore.length > 0) {
        options.ignore = configIgnore.concat(
          options.ignore.filter(function (item) {
            return configIgnore.indexOf(item) === -1;
          })
        );
      }
