...
config.load = function (settings, ready) {
  reset();
  var config = this;
  // Signal to load() that CLI ignore should be combined with config file
  // ignore values (additive), not replace them. This flag is only set when
  // loading through the real config.load entry point (CLI usage).
  config._combineIgnore = true;
  load(settings, config.options, config, function (options) {
    delete config._combineIgnore;
