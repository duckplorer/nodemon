...
var cancelDebounce = require('./monitor/watch').cancelDebounce;
var cli = require('./cli');
    ...
    if (config.options.stdin && config.options.restartable) {
      ...
      process.stdin.on('data', data => {
        ...
        if (str === config.options.restartable) {
          // Cancel any pending debounced restart so the manual restart
          // is immediate and not followed by a delayed duplicate.
          cancelDebounce();
          bus.emit('restart');
