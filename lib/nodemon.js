...
function nodemon(settings) {
  ...
  config.load(options, function (config) {
    ...
    // update JSON output mode based on --json flag
    utils.json = !!config.options.json;

    if (config.options.json) {

      // register event listeners for structured JSON event output
      // use nodemon.on() so handlers are tracked and cleaned up on reset
      ...
      nodemon.on('exit', function (signal) {
        console.log(JSON.stringify({
          type: 'event',
          event: 'exit',
          signal: signal || null,
      ...
      // use a guard to emit quit only once — run.js re-invokes quit
      // listeners without arguments, which would cause a duplicate
      var quitEmitted = false;
      ...
      nodemon.on('watching', function (file) {
        console.log(JSON.stringify({
          type: 'event',
          event: 'watching',
          file: file,
