...
// When a manual restart occurs (e.g. user types 'rs'), cancel any pending
// debounced file-change restart so the manual restart is immediate and
// a stale debounce timer does not trigger a second restart afterwards.
bus.on('restart', function (files) {
  if ((!files || files.length === 0) && debouncedBus) {
    debouncedBus.cancel();
  }
});
