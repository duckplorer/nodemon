...
function match(files, monitor, ext) {
  ...
  // Collect explicitly watched file paths (non-glob, non-ignore rules).
  // Files matching these paths should bypass the extension filter, since
  // they were added to --watch intentionally.
  var explicitPaths = rules.filter(function (r) {
    return r.slice(0, 1) !== '!' && !/[*?{\[]/.test(r);
  });
    ...
    for (var i = 0; i < rules.length; i++) {
      if (rules[i].slice(0, 1) === '!') {
        ...
        if (minimatch(file, rules[i], minimatchOpts)) {
          ...
          } else if (explicitPaths.indexOf(file) !== -1) {
            // if the file matches an explicitly watched file path (e.g.
            // via --watch .env), bypass the extension filter
            if (whitelist.indexOf(file) === -1) {
              whitelist.push(file);
            }
          } else {
