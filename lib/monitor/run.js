...
if (!utils.isWindows) {
  bus.once('boot', () => {
    ...
    process.once('SIGTERM', () => {
      if (child) {
        // Forward SIGTERM to the entire child process tree via psTree,
        // then quit. Using the kill() helper ensures all sub-processes
        // receive the signal, not just the direct shell wrapper.
        kill(child, 'SIGTERM', () => {
          bus.emit('quit', 143);
        });
      } else {
