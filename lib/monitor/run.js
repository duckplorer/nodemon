...
function kill(child, signal, callback) {
  ...
  if (utils.isWindows) {
    ...
    psTree(child.pid, function (err, pids) {
      ...
      // If killTimeout is configured, set up a forced SIGKILL after the
      // grace period so the child process has time to shut down cleanly
      // before being hard-killed. Fixes #30.
      var killTimer = null;
      if (config.killTimeout) {
        killTimer = setTimeout(function () {
          debug('kill timeout of %dms reached, sending SIGKILL', config.killTimeout);
          try { child.kill('SIGKILL'); } catch (e) {}
          pids.sort().forEach(function (pid) {
            exec('kill -9 ' + pid, noop);
          });
        }, config.killTimeout);
      }
